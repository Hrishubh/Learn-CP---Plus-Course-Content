...
public:
	...
	info apply(info val, int l, int r)
	{
...
class segTree {
	...
	void apply_and_pass(int cur, int l, int r)
	{
		if (lazy[cur].add)
		{
			tree[cur] = lazy[cur].apply(tree[cur], l, r);
	...
	void update(int cur, int tl, int tr, int ul, int ur, int val)
	{
		// Apply existing updates
		apply_and_pass(cur, tl, tr);
...
public:
	...
	void update(int l, int r, int val) // O(logn) -> ~4*log2(n)
	{
